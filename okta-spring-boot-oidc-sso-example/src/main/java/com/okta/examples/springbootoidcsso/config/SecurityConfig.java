...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public GrantedAuthoritiesMapper userAuthoritiesMapper() {
        return (authorities) -> {
            java.util.Set<GrantedAuthority> mappedAuthorities = new java.util.HashSet<>();

            authorities.forEach(authority -> {
                if (authority instanceof OidcUserAuthority) {
                    OidcUserAuthority oidcUserAuthority = (OidcUserAuthority) authority;
                    OidcIdToken idToken = oidcUserAuthority.getIdToken();
                    java.util.List<String> groups = idToken.getClaimAsStringList("groups");
    ...
    private LogoutSuccessHandler customOidcLogoutHandler() {
        return (HttpServletRequest request, HttpServletResponse response, Authentication authentication) -> {
            ...
            String logoutUrl = issuer + "/v1/logout" +
                    ...
                    "&post_logout_redirect_uri=" + URLEncoder.encode("http://localhost:8080/", StandardCharsets.UTF_8.name());
